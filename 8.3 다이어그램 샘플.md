...
flowchart TD
    A[검색 요청 시작] --> B{자동 감지 활성화?}
    B -->|Yes| C[쿼리 분석 및 카테고리 자동 감지]
    B -->|No| D[명시적 카테고리 필터 적용]
    C --> E[쿼리 임베딩 생성]
    D --> E
    
    E --> F[BGEM3 모델로 Dense/Sparse 벡터 생성]
    F --> G[QdrantManager: 초기 DB 검색]
    ...
    L -->|구조화된 문서| M[Context-Aware 경로]
    L -->|비구조화 문서| N[Simple 경로]
    
    M --> O[RRF 융합]
    O --> P[부모 문맥 재정렬]
    P --> Q[다양성 필터링]
    
    N --> R[Dense/Sparse 가중합]
    R --> S[점수 정규화]
    ...
    T --> U[카테고리별 필드 추출]
    U --> V{추출 레지스트리}
    V -->|행정규칙| W[url_link, rule_name 추출]
    V -->|판례| X[url_link, 사건명 추출]
    V -->|법령| Y[법령상세링크, 법령명 추출]
    V -->|규정| Z[규정명, 가이드라인명 추출]
    ...
    AA --> BB{API 타입}
    BB -->|FastMCP| CC[MCP 프로토콜 응답]
    BB -->|HTTP GET| DD[REST API 응답]
    BB -->|HTTP POST| EE[POST API 응답]
    BB -->|SSE| FF[Server-Sent Events 응답]
...
![n8n context7 이미지](https://raw.githubusercontent.com/HunHee-Joung/Study/main/img/20250904_Agentic_Rag.png)
...
stateDiagram-v2
    ...
    RetrievalAgent --> VectorSearchA
    RetrievalAgent --> VectorSearchB
    RetrievalAgent --> Calculator
    RetrievalAgent --> WebSearch

    VectorSearchA --> CollectionA
    VectorSearchB --> CollectionB

    CollectionA --> RetrievalAgent
    CollectionB --> RetrievalAgent
    Calculator --> RetrievalAgent
    WebSearch --> RetrievalAgent
